...
This command takes an existing plan (from `/mg:plan`) and enhances each section with parallel research agents. Each major element gets its own dedicated research sub-agent to find:
...
# 3. magical-gazelle plugin skills
ls ~/.claude/plugins/cache/*/magical-gazelle/*/skills/
...
**Important:** Check EVERY source. Don't assume magical-gazelle is the only plugin. Use skills from ANY installed plugin that's relevant.
...
<thinking>
Check for documented learnings from /mg:compound. These are solved problems stored as markdown files. Spawn a sub-agent for each learning to check if it's relevant.
</thinking>

**LEARNINGS LOCATION - Check these exact folders:**

```
docs/solutions/           <-- PRIMARY: Project-level learnings (created by /mg:compound)
├── performance-issues/
│   └── *.md
├── debugging-patterns/
│   └── *.md
...
```
mcp__plugin_magical-gazelle_context7__resolve-library-id: Find library ID for [framework]
mcp__plugin_magical-gazelle_context7__query-docs: Query documentation for specific patterns
```
...
# 3. magical-gazelle plugin agents (all subdirectories)
find ~/.claude/plugins/cache/*/magical-gazelle/*/agents -name "*.md" 2>/dev/null
...
- magical-gazelle plugin (but SKIP workflow/ agents - only use review/, research/, design/, docs/)
- ALL other installed plugins (agent-sdk-dev, frontend-design, etc.)
- Any local plugins

**For magical-gazelle plugin specifically:**
...
Research agents (like `framework-docs-researcher`, `repo-research-analyst`) should also be run for relevant plan sections.
...
2. **Learnings/Solutions sub-agents** - Relevant documented learnings from /mg:compound
...
2. **Run `/mg:review`** - Get feedback from reviewers on enhanced plan
3. **Start `/mg:work`** - Begin implementing this enhanced plan
4. **Deepen further** - Run another round of research on specific sections
5. **Revert** - Restore original plan (if backup exists)

Based on selection:
- **View diff** → Run `git diff [plan_path]` or show before/after
- **`/mg:review`** → Call the /mg:review command with the plan file path
- **`/mg:work`** → Call the /mg:work command with the plan file path

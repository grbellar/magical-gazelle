...
|------|---------|-------------------|----------|----------|
| 1. Add column | `python manage.py migrate` | < 1 min | N/A | Drop column |
| 2. Backfill data | `python manage.py backfill` | ~10 min | 1000 rows | Restore from backup |
...
```python
# Quick sanity check
Record.objects.filter(new_column__isnull=True).exclude(old_column__isnull=True).count()
# Expected: 0

# Spot check random records
Record.objects.order_by('?')[:10].values_list('old_column', 'new_column')
